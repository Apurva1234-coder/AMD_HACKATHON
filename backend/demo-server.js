/**
 * Complete Crisis Classification API Test
 * This demonstrates the full system functionality
 */

const express = require('express');
const cors = require('cors');
const helmet = require('helmet');
const { GoogleGenerativeAI } = require('@google/generative-ai');
const rateLimit = require('express-rate-limit');

// Load environment variables
require('dotenv').config();

const app = express();
const PORT = process.env.PORT || 3001;

// Security and middleware
app.use(helmet());
app.use(cors());
app.use(express.json({ limit: '10mb' }));

// Rate limiting
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 100, // limit each IP to 100 requests per windowMs
  message: { error: 'Too many requests, please try again later.' }
});
app.use(limiter);

// Initialize Gemini AI (will fallback if not working)
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });

const VALID_CATEGORIES = ['Emergency', 'Medical', 'Resource', 'Safe', 'General'];

// Smart fallback classification function
function classifyWithFallback(message) {
  const text = message.toLowerCase();
  
  const emergencyKeywords = [
    'emergency', 'urgent', 'help', 'trapped', 'fire', 'explosion', 'collapsed', 
    'earthquake', 'flood', 'hurricane', 'tornado', 'attack', 'violence',
    'danger', 'rescue', 'stuck', 'falling', 'burning', 'drowning', 'accident',
    'crash', 'collision', 'gunshot', 'shooting', 'bomb', 'gas leak'
  ];
  
  const medicalKeywords = [
    'medical', 'hospital', 'doctor', 'nurse', 'ambulance', 'injured', 'hurt',
    'bleeding', 'pain', 'sick', 'medicine', 'pills', 'surgery', 'broken',
    'fracture', 'wound', 'infection', 'fever', 'unconscious', 'breathing',
    'heart attack', 'stroke', 'allergic', 'diabetic', 'seizure', 'overdose'
  ];
  
  const resourceKeywords = [
    'food', 'water', 'shelter', 'clothing', 'supplies', 'need', 'looking for',
    'hungry', 'thirsty', 'homeless', 'cold', 'warm', 'blanket', 'tent',
    'relief', 'aid', 'assistance', 'donation', 'volunteer', 'resources',
    'electricity', 'power', 'fuel', 'transportation', 'phone', 'communication'
  ];
  
  const safetyKeywords = [
    'safe', 'okay', 'fine', 'secure', 'protected', 'shelter', 'evacuated',
    'rescued', 'all clear', 'no injuries', 'no damage', 'stable', 'recovered',
    'unharmed', 'intact', 'operational', 'functioning', 'normal'
  ];
  
  const scores = {
    Emergency: emergencyKeywords.reduce((count, keyword) => 
      count + (text.includes(keyword) ? 1 : 0), 0),
    Medical: medicalKeywords.reduce((count, keyword) => 
      count + (text.includes(keyword) ? 1 : 0), 0),
    Resource: resourceKeywords.reduce((count, keyword) => 
      count + (text.includes(keyword) ? 1 : 0), 0),
    Safe: safetyKeywords.reduce((count, keyword) => 
      count + (text.includes(keyword) ? 1 : 0), 0)
  };
  
  const maxScore = Math.max(...Object.values(scores));
  const bestCategory = Object.keys(scores).find(cat => scores[cat] === maxScore);
  
  return maxScore > 0 ? bestCategory : 'General';
}

// Main classification function
async function classifyCrisisMessage(message) {
  try {
    const prompt = `Classify this crisis message into ONE category: Emergency, Medical, Resource, Safe, General. Response format: just the category name only.\n\nMessage: "${message}"`;
    
    const result = await model.generateContent({
      contents: [{ parts: [{ text: prompt }] }],
      generationConfig: {
        maxOutputTokens: 10,
        temperature: 0.1,
      },
    });
    
    const response = await result.response;
    const text = response.text().trim();
    
    const category = VALID_CATEGORIES.find(cat => 
      text.toLowerCase().includes(cat.toLowerCase())
    );
    
    return category || classifyWithFallback(message);
    
  } catch (error) {
    console.log('🔄 Using fallback classification (Gemini unavailable)');
    return classifyWithFallback(message);
  }
}

// Routes
app.get('/', (req, res) => {
  res.json({
    message: 'Crisis Mapping AI Classification API',
    version: '1.0.0',
    status: 'operational',
    features: [
      'Gemini AI-powered classification',
      'Smart fallback keyword matching',
      'Production-ready error handling',
      'Rate limiting & security'
    ],
    endpoints: {
      'POST /api/classify': 'Classify crisis messages',
      'GET /health': 'Health check',
      'GET /test': 'Run classification tests'
    }
  });
});

app.get('/health', (req, res) => {
  res.json({ 
    status: 'healthy', 
    timestamp: new Date().toISOString(),
    gemini_configured: !!process.env.GEMINI_API_KEY
  });
});

// Test endpoint
app.get('/test', async (req, res) => {
  const testMessages = [
    "Building collapsed, people trapped inside!",
    "Need medical supplies, someone is bleeding",
    "Looking for food and water for 10 people",
    "We are safe in the shelter, no injuries",
    "What's the weather forecast for tomorrow?"
  ];
  
  const results = [];
  
  for (const message of testMessages) {
    const category = await classifyCrisisMessage(message);
    results.push({ message, category });
  }
  
  res.json({
    test_results: results,
    system_status: 'All tests completed successfully'
  });
});

// Crisis message classification endpoint
app.post('/api/classify', async (req, res) => {
  try {
    const { message } = req.body;
    
    if (!message || typeof message !== 'string' || message.trim().length === 0) {
      return res.status(400).json({
        error: 'Message is required and must be a non-empty string',
        category: 'General'
      });
    }
    
    if (message.length > 500) {
      return res.status(400).json({
        error: 'Message too long (max 500 characters)',
        category: 'General'
      });
    }
    
    const category = await classifyCrisisMessage(message.trim());
    res.json({ category });
    
  } catch (error) {
    console.error('Classification endpoint error:', error);
    res.status(500).json({
      error: 'Classification service temporarily unavailable',
      category: 'General'
    });
  }
});

// Error handling
app.use((err, req, res, next) => {
  console.error(err.stack);
  res.status(500).json({
    error: 'Something went wrong!',
    category: 'General'
  });
});

app.use((req, res) => {
  res.status(404).json({
    error: 'Endpoint not found',
    available_endpoints: ['GET /', 'GET /health', 'POST /api/classify', 'GET /test']
  });
});

// Auto-run tests on startup
async function runStartupTests() {
  console.log('\n🧪 Running startup tests...\n');
  
  const testMessages = [
    { text: "Building collapsed, people trapped!", expected: "Emergency" },
    { text: "Need medical assistance", expected: "Medical" },
    { text: "Looking for food and water", expected: "Resource" },
    { text: "All safe here", expected: "Safe" },
    { text: "What time is it?", expected: "General" }
  ];
  
  for (const test of testMessages) {
    const result = await classifyCrisisMessage(test.text);
    const status = result === test.expected ? '✅' : '⚠️';
    console.log(`${status} "${test.text}" → ${result}`);
  }
  
  console.log('\n🎉 Tests completed!\n');
}

// Start server
const server = app.listen(PORT, async () => {
  console.log(`\\n🚀 Crisis Mapping Server running on port ${PORT}`);
  console.log(`📍 Access: http://localhost:${PORT}`);
  console.log(`🔗 Health: http://localhost:${PORT}/health`);
  console.log(`🧪 Tests: http://localhost:${PORT}/test`);
  console.log(`🔑 Gemini API: ${process.env.GEMINI_API_KEY ? 'Configured ✅' : 'Missing ⚠️'}`);
  
  try {
    await runStartupTests();
    console.log('🌟 Server ready for production!\\n');
  } catch (error) {
    console.log('⚠️ Startup tests failed, but server is running with fallback\\n');
  }
});

module.exports = app;